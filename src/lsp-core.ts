 ...
 *
 * Manages LSP server lifecycles, file synchronization, and diagnostic collection.
 * One server per (language, project-root) pair, reused across turns.
 */
...
function simpleSpawn(bin: string, args: string[] = ["--stdio"]) {
  return async (root: string) => {
    ...
    return { process: spawn(cmd, args, { cwd: root, stdio: ["pipe", "pipe", "pipe"] as const }) };
  };
}

async function spawnChecked(cmd: string, args: string[], cwd: string): Promise<ChildProcessWithoutNullStreams | undefined> {
  try {
    const child = spawn(cmd, args, { cwd, stdio: ["pipe", "pipe", "pipe"] as const });
    ...
    return await new Promise((resolve) => {
      ...
      // If the process survives 200ms, it's likely running
...
function findRootKotlin(file: string, cwd: string): string | undefined {
  ...
  return findRoot(file, cwd, [
    "build.gradle.kts", "build.gradle", "gradlew", "gradlew.bat",
...
async function ensureJetBrainsKotlinLspInstalled(): Promise<string | undefined> {
  ...
  try {
    ...
    const platform = process.platform === "darwin" ? "mac" : process.platform === "linux" ? "linux" : process.platform === "win32" ? "win" : process.platform;
    ...
    const assetName = `kotlin-lsp-${version}-${platform}-${arch}.zip`;
...
async function spawnRubyLsp(root: string): Promise<ChildProcessWithoutNullStreams | undefined> {
  const direct = which("ruby-lsp");
  if (direct) return spawnWithFallback(direct, [[], ["--stdio"]], root);
  return undefined;
}
...
export const LSP_SERVERS: LSPServerConfig[] = [
  {
    ...
    spawn: async (root) => {
      ...
      return { process: spawn(dart, ["language-server", "--protocol=lsp"], { cwd: root, stdio: ["pipe", "pipe", "pipe"] as const }) };
    },
  },
  {
    id: "typescript",
    ...
    spawn: async (root) => {
      ...
      if (!cmd) return undefined;
      return { process: spawn(cmd, ["--stdio"], { cwd: root, stdio: ["pipe", "pipe", "pipe"] as const }) };
    },
  },
  ...
  { id: "rust-analyzer", extensions: [".rs"], findRoot: (f, cwd) => findRoot(f, cwd, ["Cargo.toml"]), spawn: simpleSpawn("rust-analyzer", []) },
  {
    ...
    findRoot: (f, cwd) => findRoot(f, cwd, ["Gemfile", ".ruby-version", "Rakefile"]),
    spawn: async (root) => {
      const proc = await spawnRubyLsp(root);
      if (!proc) return undefined;
      return { process: proc };
    },
  },
...
export async function shutdownManager(): Promise<void> {
  ...
  // Clear pointers first so new requests never receive a shutting-down manager
  sharedManager = null;
  managerCwd = null;
...
export class LSPManager {
  ...
  private async initClient(config: LSPServerConfig, root: string): Promise<LSPClient | undefined> {
    ...
    try {
      ...
      conn.onNotification("textDocument/publishDiagnostics", (params: { uri: string; diagnostics: Diagnostic[] }) => {
        ...
        // Notify both raw and normalized paths (macOS /private/var vs /var)
        const listeners1 = client.listeners.get(fp);
        const listeners2 = fp !== fpRaw ? client.listeners.get(fpRaw) : undefined;
        listeners1?.slice().forEach((fn) => { try { fn(); } catch {} });
        listeners2?.slice().forEach((fn) => { try { fn(); } catch {} });
      });
  ...
  private explainNoLsp(absPath: string): string {
    ...
    if (ext === ".rb" || ext === ".erb") {
      const root = findRoot(absPath, this.cwd, ["Gemfile", ".ruby-version", "Rakefile"]);
      if (!root) return "No Ruby project root detected (looked for Gemfile, .ruby-version, Rakefile under cwd)";
      if (!which("ruby-lsp")) return "ruby-lsp not found. Install via `gem install ruby-lsp`.";
      const k = this.key("ruby", root);
      if (this.broken.has(k)) return `ruby-lsp failed to initialize for root: ${root}`;
      return `Ruby LSP unavailable for root: ${root}`;
    }
    ...
    for (const client of clients) {
      ...
      try {
        if (state) {
          ...
          // Some servers (e.g. kotlin-language-server) only start diagnostics after didChange,
          // so we open at version 0 then immediately send a full-content didChange at version 1.
  ...
  private waitForDiagnostics(client: LSPClient, absPath: string, timeoutMs: number, isNew: boolean): Promise<boolean> {
    return new Promise((resolve) => {
      ...
      // Some servers publish diagnostics multiple times (empty first, then real).
      // For new documents with empty diagnostics, debounce briefly.
      listener = () => {
        if (resolved) return;
  ...
  private async pullDiagnostics(client: LSPClient, absPath: string, uri: string): Promise<{ diagnostics: Diagnostic[]; responded: boolean }> {
    ...
    // Fallback: workspace diagnostic request
    try {
  ...
  async getDiagnosticsForFiles(files: string[], timeoutMs: number): Promise<FileDiagnosticsResult> {
    ...
    // Cleanup files that were only opened for this scan
  ...
  const visit = (items: DocumentSymbol[]) => {
    for (const sym of items) {
      ...
      if (pos && typeof pos.line === "number" && typeof pos.character === "number") {
        if (!exact && nameLower === q) exact = pos;
        if (!partial) partial = pos;
      }
      if (sym?.children?.length) visit(sym.children);
    }
  };
