 ...
 *
 * Provides on-demand LSP queries: definitions, references, hover, signature help,
...
// ---------------------------------------------------------------------------
// Argument normalization (handles both pi runtime versions)
// ---------------------------------------------------------------------------
...
function isAbortSignalLike(value: unknown): value is AbortSignal {
  return !!value && typeof value === "object" && "aborted" in value && typeof (value as any).addEventListener === "function";
}
...
function formatSignature(help: any): string {
  ...
  if (sig.parameters?.length) {
    const params = sig.parameters
      .map((p: any) => typeof p.label === "string" ? p.label : Array.isArray(p.label) ? p.label.join("-") : "")
      .filter(Boolean);
