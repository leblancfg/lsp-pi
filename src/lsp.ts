 ...
 *
 * Provides automatic diagnostics feedback. Default mode: agent_end (runs once
 * after all tool calls complete). Can also run after each write/edit or be disabled.
 */
...
export default function (pi: ExtensionAPI) {
  ...
  let touchedFiles: Map<string, boolean> = new Map();
  ...
  function getGlobalHookMode(): HookMode | undefined {
    ...
    const lspSettings = settings[SETTINGS_NAMESPACE] as { hookMode?: unknown; hookEnabled?: unknown } | undefined;
    const normalized = normalizeHookMode(lspSettings?.hookMode);
    if (normalized) return normalized;

    // Legacy boolean toggle
    if (typeof lspSettings?.hookEnabled === "boolean") return lspSettings.hookEnabled ? "edit_write" : "disabled";
    return undefined;
  }
  ...
  function restoreHookState(ctx: ExtensionContext): void {
    ...
    if (entry?.scope === "session") {
      ...
      if (normalized) { hookMode = normalized; hookScope = "session"; return; }
  ...
  function messageContentToText(content: unknown): string {
    ...
    if (Array.isArray(content)) {
      return content
        .map((item) =>
          item && typeof item === "object" && "type" in item && (item as any).type === "text"
  ...
  pi.registerCommand("lsp", {
    ...
    handler: async (_args, ctx) => {
      ...
      const modeOptions = (["edit_write", "agent_end", "disabled"] as HookMode[]).map((mode) => ({
        mode,
        label: mode === hookMode ? `${MODE_LABELS[mode]}${currentMark}` : MODE_LABELS[mode],
      }));
      ...
      if (scope === "global") {
        ...
        if (!ok) { ctx.ui.notify("Failed to update global settings", "error"); return; }
      }
      ...
      updateLspStatus();
      ctx.ui.notify(`LSP hook: ${MODE_LABELS[hookMode]} (${hookScope})`, "info");
    },
  });
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    // Warm up the LSP for the detected project type
  ...
  pi.on("session_switch", async (_event, ctx) => { restoreHookState(ctx); updateLspStatus(); });
  pi.on("session_tree", async (_event, ctx) => { restoreHookState(ctx); updateLspStatus(); });
  pi.on("session_fork", async (_event, ctx) => { restoreHookState(ctx); updateLspStatus(); });
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    if (event.toolName === "lsp") {
      clearIdleShutdownTimer();
      for (const file of extractLspFiles(input)) ensureActiveClientForFile(file, ctx.cwd);
      return;
    }
  ...
  function agentWasAborted(event: any): boolean {
    ...
    return messages.some(
      (m: any) => m?.role === "assistant" && (m?.stopReason === "aborted" || m?.stopReason === "error"),
    );
  }
  ...
  pi.on("agent_end", async (event, ctx) => {
    try {
      ...
      if (agentWasAborted(event)) { touchedFiles.clear(); return; }
      ...
      try {
        ...
        for (const [filePath, includeWarnings] of files) {
          ...
          if (!ctx.isIdle() || ctx.hasPendingMessages()) { abort.abort(); return; }
        ...
        if (outputs.length) {
          pi.sendMessage(
            { customType: "lsp-diagnostics", content: outputs.join("\n"), display: true },
  ...
  pi.on("tool_result", async (event, ctx) => {
    ...
    // edit_write mode: append diagnostics to the tool result
    const includeWarnings = event.toolName === "write";
